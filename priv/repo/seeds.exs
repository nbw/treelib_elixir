...
defmodule Treelib.DbSeeder do
  ...
  def insert_tree(
    ...
    species = Species.active |> Repo.all
    (1..15)
    |> Enum.each(fn c ->
      species_ids =
        Enum.take_random(species, c)
        |> Enum.map(fn s -> s.id end)

      insert_contributor(species_ids)
    end)
  end
  ...
  def insert_contributor(species_ids \\ []) do
    %Contributor{}
    |> Contributor.changeset(%{
      first_name: lorem_first_name(),
      last_name: lorem_last_name(),
      description: lorem_description(),
  ...
  defp lorem_first_name, do: Faker.Name.first_name()

  defp lorem_last_name, do: Faker.Name.last_name()
