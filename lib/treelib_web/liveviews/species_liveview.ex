defmodule TreelibWeb.SpeciesLiveView do
  ...
  def mount(_, _, socket) do
    species =
      SpeciesManager.species_list()
      |> Enum.map(fn %{
                       genus_name: g_n,
                       genus_common_name: g_cn,
                       species_name: s_n,
                       species_common_name: s_cn
                     } = species ->
  ...
  def handle_event("search", %{"search_field" => %{"query" => query}}, socket) do
    ...
    filtered_species =
      case query do
        ...
        q ->
          Enum.filter(species, fn %{searchable_string: s} ->
  ...
  def handle_event("sort", %{"dir" => "up", "col" => col}, socket) do
    ...
    species =
      Enum.sort(socket.assigns.species, &(String.downcase(&1[key]) < String.downcase(&2[key])))

    filtered =
      Enum.sort(
        socket.assigns.filtered_species,
        &(String.downcase(&1[key]) < String.downcase(&2[key]))
      )
